 ...
 * @version # 20170608:修改为从离线计算上报
 */
public class InnerSspInventoryReportJob extends AbstractExtOptJob{
	...
	public void process(String time) {
		logger.info("###############innerssp 实时库存上报处理开始 ###########################");
...
//		preSql = preSql.replaceAll("@time", dayTime);
		...
		List<Map<String,Object>> invList =  hiveJt.queryForList(newSql);
		...
		if(null!=formatDataMap && formatDataMap.size()>0){
			...
			while(mediaIt.hasNext()){
				...
				logger.info("重新组织数据媒体["+mediaId+"],oemid=["+oemId+"] 下广告位数据【"+adpIdMap.size()+"】条!");
				//媒体方广告位
				...
				while(adpidIt.hasNext()){
					...
					try {
						...
						for(Map<String,Object> data:datas){
							String areaCode = data.get("t.cityid").toString();
							String playnum = (null!=data.get("t.playnum"))?data.get("t.playnum").toString():"0";
	...
	private Map<String, Object> formatData(List<Map<String, Object>> invList) {
		logger.info("-------------开始格式化库存数据-----------------");
		Map<String, Object> formatData = new HashMap<String,Object>();			
		for(Map<String,Object> data:invList){
		  logger.debug("-----orgdata:["+data.toString()+"]-----------------");
		  String mediaId = (null!=data.get("t.mediaid"))?data.get("t.mediaid").toString():"";
		  String oemid = (null!=data.get("t.oemid"))?data.get("t.oemid").toString():"";
		  String adpid = (null!=data.get("t.adpid"))?data.get("t.adpid").toString():"";
		  logger.debug("-----invdata:[mediaId="+mediaId+",oemid="+oemid+",adpid="+adpid+"]-----------------");
